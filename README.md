...
activity for a month and renders it as a readable text summary — a plain-language
...
2. **`report.py`** — reads the saved `data/` files (never the network) and writes
   a `webex-summary-YYYY-MM.txt` grouped as:
...
./venv/bin/python report.py      # render webex-summary-<month>.txt
```

Or target an explicit period:
...
- **`settings.yaml`** — stable settings:
  ...
  | `message_char_limit` | max characters shown per message before truncation |
...
- **Report is a pure re-render.** Re-running overwrites `webex-summary-<month>.txt`.
...
`data/` and the generated `webex-summary-*.txt` contain real message text — which
